...
export function useCanvas({Worker, style = '', id = ''} :useCanvasArgs) {
  ...
  const post = useCallback((message: any, transfer?: any[]) => workerRef.current?.postMessage(message, transfer ?? []), [])
  ...
  useEffect(() => {
    ...
    const handleTouchMove = ({changedTouches, touches} : TouchEvent) => {
      if (touches.length === 1) {
        ...
        const {clientWidth, clientHeight} = target as HTMLElement
        const {x, y} = screenToViewPort({offsetX: clientX, offsetY: clientY, clientWidth, clientHeight})
        post({mouse: {x, y}})
      }
    }
